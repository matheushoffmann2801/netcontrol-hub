...
app.post('/auth/login', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: admin.id, role: 'ADMIN' }, JWT_ADMIN_SECRET, { expiresIn: '1d' } // Token válido por 1 dia
        );
        res.json({
...
app.get('/companies/:id', async (req, res) => {
    try {
        ...
        if (companyCopy.licenses && companyCopy.licenses.length > 0 && typeof companyCopy.licenses[0].modules === 'string') {
            companyCopy.licenses = companyCopy.licenses.map((l) => ({ ...l, modules: JSON.parse(l.modules) }));
        }
